...
class RoomsFacilitiesRepository(BaseRepository):
    ...
    async def update_facilities(
        ...
        if facilities_to_add:
            add_facilities_stmt = insert(self.model).values([{"room_id": room_id, "facility_id": f_id} for f_id in facilities_to_add])
            await self.session.execute(add_facilities_stmt)

        if facilities_to_delete:
            delete_facilities_stmt = delete(self.model).filter_by(room_id=room_id).filter(self.model.facility_id.in_(facilities_to_delete))
